...
combined_1 <-
  bind_rows(
    eighteen %>% mutate(category = "Wrapped playlist 2018") %>% slice_head(n = 20),
    nineteen %>% mutate(category = "Wrapped playlist 2019") %>% slice_head(n = 20),
    twenty %>% mutate(category = "Wrapped playlist 2020") %>% slice_head(n = 20)
  )
combined <-
  bind_rows(
...
```{r}
combined_cv <- combined_features %>% vfold_cv(5)
```
...
```{r}
combined_features %>%
  ggplot(aes(x = energy, y = valence, colour = category, size = danceability)) +
...
scatterplotyears <- combined %>%
  ...
  geom_point(color = '#008ae6', alpha = 0.6) +
